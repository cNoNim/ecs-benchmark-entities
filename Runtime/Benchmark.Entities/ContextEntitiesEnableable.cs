...
public partial class ContextEntitiesEnableable : ContextBase
{
	private BenchmarkSystemGroup? _group;
	private World?                _world;

	public ContextEntitiesEnableable()
		: base("Entities Enableable") {}
	...
	private partial struct SpawnSystem : ISystem
	{
		...
		public void OnUpdate(ref SystemState state)
		{
			...
			foreach (var entity in entities)
			{
				...
				state.EntityManager.SetComponentEnabled<ToggleDead>(entity, false);
			}
		}
	}
	...
	private partial struct UpdateVelocitySystem : ISystem
	{
		...
		private ComponentTypeHandle<ToggleDead>   _deadHandle;
		...
		public void OnCreate(ref SystemState state)
		{
			...
			_deadHandle     = state.GetComponentTypeHandle<ToggleDead>(true);
		}
		...
		public void OnUpdate(ref SystemState state)
		{
			...
			foreach (var chunk in _query.ToArchetypeChunkArray(state.WorldUpdateAllocator))
			{
				...
				{
					if (chunk.IsComponentEnabled(ref _deadHandle, n))
						continue;
	...
	private partial struct DamageSystem : ISystem
	{
		...
		private ComponentLookup<ToggleDead>       _deadLookup;
		...
		public void OnCreate(ref SystemState state)
		{
			...
			_deadLookup   = state.GetComponentLookup<ToggleDead>();
	...
	private partial struct KillSystem : ISystem
	{
		...
		public void OnCreate(ref SystemState state)
		{
			...
			_dataHandle   = state.GetComponentTypeHandle<CompData>(true);
			_deadHandle   = state.GetComponentTypeHandle<ToggleDead>();
		}
		...
		public void OnUpdate(ref SystemState state)
		{
			...
			foreach (var chunk in _query.ToArchetypeChunkArray(state.WorldUpdateAllocator))
			{
				...
				for (var n = 0; n < units.Length; n++)
				{
					...
					var unit = units[n].V;
					chunk.SetComponentEnabled(ref _deadHandle, n, true);
					var data = datas[n].V;
	...
	private partial struct RespawnSystem : ISystem
	{
		...
		public void OnCreate(ref SystemState state)
		{
			_query = new EntityQueryBuilder(state.WorldUpdateAllocator).WithAll<CompUnit, CompData, ToggleDead>()
																	   .Build(ref state);
